...
**BA-retune across the catalog (spec-022 US2 extended, 2026-05-12 evening)**. Every system with both `val_predictions.csv` and `test_predictions.csv` was re-tuned: threshold sweep on val to maximise balanced accuracy, applied to test once (Constitution IV compliant). 280 systems retuned; output at `evaluation/balanced_metrics_ba_tuned_summary.csv`. Companion utility: `evaluation/retune_all_by_ba.py`. Bug fix: `balanced_metrics.py` now reads `enroll_val_metrics.json` (enrollment-system threshold file naming — previously fell back to default-0.5 which produced wrong F1-tuned values for BabAR/VTC/VBx/EEND-EDA/Sortformer/USC-SAIL enrollment rows; fixed). Headline BA-tuned table promoted to chapter 5 of `thesis_v2/`; F1-tuned values preserved in appendix C's retune-audit table with $\Delta$F1 / $\Delta$BA columns. **Mean catalog trade-off: $-0.029$ F1 for $+0.054$ BA** when retuning from F1-max to BA-max. Most dramatic retunes: Whisper pseudo-frame BA $0.552 \to 0.771$ ($+0.219$) and WavLM pseudo-frame BA $0.538 \to 0.742$ ($+0.204$) — both had been at recall$\approx$0.99 operating points that the F1-max threshold was hiding.
