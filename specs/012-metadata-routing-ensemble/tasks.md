...
- [x] T024 short_voc_head — VERIFIED: `mil/mil_results/short_voc_head/test_metrics_short_voc_clips.json` exists with `n_recovered=0, n_hurt=0, n=341, F1=0.906, AUROC=0.887, AUPRC=0.967`. Note: head was no-op in practice (recovered/hurt both 0) — clip-level distribution is the same before and after; documented as a null result.
- [x] T025 baseline_seen_child — RESOLVED via **path investigation**: results live at `./baseline_results_seen_child/` (repo root) **NOT** `./baselines/baseline_results_seen_child/` (which the SLURM script `mkdir -p`'d but Python's CWD-relative writes ignored). 14 variant dirs (one more than the spec's 13: `whisper_mean`, `whisper_attn`, `whisper_attn_lw`, `whisper_attn_aug`, `whisper_attn_aug_ptt`, `whisper_attn_ptt`, `whisper_attn_unfreeze2`, `whisper_stats_lw`, `wavlm_mean`, `wavlm_attn`, `wavlm_attn_lw`, `wavlm_stats_lw`, `fused_attn`, `fused_attn_unfreeze2`) — each with full schema (`test_metrics_tuned.json`, `test_metrics_by_timepoint.csv`, `val_metrics_tuned.json`, `val_predictions.csv`, `test_predictions.csv`, `config.json`, `best_model.pt`, `training_history.csv`). Job 12770942 ran 13:13 hours and COMPLETED cleanly. Path mismatch was the only issue. **Action item if not yet done**: add §4b cross-child vs seen-child comparison table to `results_summary.md`; current §12 entries appear to use cross-child only.
- [x] T026 Age-band prototype fix — VERIFIED: 3 git commits cover the bug+propagation (`9003431` Fix age-band prototype bug in vtc_combined.py + pyannote_ecapa.py; `478ad4f` Fix stale prototype-missing warning; `7a84f7e` Add AV fusion results with age-band-specific BabAR prototypes). `results_summary.md` has 28 mentions of age-band/14-month/36-month, confirming new rows were added (not just commits). Sub-items (a)-(e) all reflected in results_summary.md edits and the modified files in `git status` for `ensemble_runs/metadata_stack/*`.
- [x] T027 synth_scene_gen — VERIFIED: 5000 WAV + 5000 RTTM + 5000 JSON in `synth_results/synthetic_scenes/`; `synth_results/manifests/synthetic_manifest.csv` has 5000 rows. `results_summary.md` has 15 mentions of synth/synthetic confirming §7 was updated.
