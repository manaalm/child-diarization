...
class Config:
    ...
    out_dir: str = "/orcd/scratch/orcd/008/manaal/child-adult-diarization/whisper-modeling/all_ages_splits"
    seed: int = 42
...
def normalize_timepoint(tp) -> str:
    if pd.isna(tp):
        return "other"
    tp = str(tp).strip()
    if tp in {"14_month", "36_month"}:
        return tp
    return tp if tp else "other"
...
def make_all_ages_split(cfg: Config):
    ...
    # Split within each child only — no timepoint stratification.
    for child_id, sub in df.groupby("child_id", dropna=False):
        if len(sub) < cfg.min_clips_per_child:
            dropped_children.append({"child_id": str(child_id), "n_rows": int(len(sub))})
            continue
    ...
    summary = {
        "seed": cfg.seed,
        "min_clips_per_child": cfg.min_clips_per_child,
        ...
        "labels_train": {int(k): int(v) for k, v in train_df["label"].value_counts().to_dict().items()},
        "labels_val": {int(k): int(v) for k, v in val_df["label"].value_counts().to_dict().items()},
        "labels_test": {int(k): int(v) for k, v in test_df["label"].value_counts().to_dict().items()},
