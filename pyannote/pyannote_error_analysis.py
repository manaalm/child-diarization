"""
Error analysis for PyAnnote + ECAPA enrollment experiment.

Mirrors the structure of the BabAR combined error analysis so results
are directly comparable across experiments.

Usage:
    python pyannote_error_analysis.py \
        --results-dir /home/manaal/orcd/scratch/child-adult-diarization/pyannote/pyannote_enrollment_runs \
        --output-dir /home/manaal/orcd/scratch/child-adult-diarization/pyannote/pyannote_enrollment_runs/error_analysis
"""
...
def main():
    ...
    # Ensure pred_label exists (re-derive from threshold if needed)
    if "pred_label" not in df.columns:
        # Try to load threshold from val_metrics.json
        val_metrics_path = os.path.join(args.results_dir, "val_metrics.json")
        if os.path.exists(val_metrics_path):
            with open(val_metrics_path, "r") as f:
                val_info = json.load(f)
            threshold = val_info.get("threshold", 0.5)
        else:
            threshold = 0.5
        print(f"Deriving pred_label from threshold={threshold:.3f}")
    ...
    # (PyAnnote enrollment only has 'prob'; BabAR has richer features.
    #  We check so sections that reference them degrade gracefully.)
    # ---------------------------------------------------------
    ...
    # Always include prob as a "feature" for distribution analysis
    if "prob" not in feature_cols:
    ...
    # Print key features (use what's available)
    highlight_feats = [f for f in ["kchi_total_dur", "phon_n_unique", "sim_weighted_mean", "prob"]
    ...
    if len(fp_df) > 0:
        ...
        # Print diarizer features if available, otherwise just prob
        for col in ["kchi_total_dur", "kchi_n_segments", "phon_n_total", "phon_n_unique"]:
            if col in fp_df.columns:
                print(f"FP mean {col}: {fp_df[col].mean():.2f}")
        ...
        if len(high_conf_fp) > 0:
            for col in ["kchi_total_dur", "phon_n_total"]:
                if col in high_conf_fp.columns:
                    print(f"  Mean {col}: {high_conf_fp[col].mean():.2f}")
    ...
    if len(fn_df) > 0:
        ...
        for col in ["kchi_total_dur", "kchi_n_segments", "phon_n_total"]:
            if col in fn_df.columns:
                print(f"FN mean {col}: {fn_df[col].mean():.2f}")
        print(f"FN mean prob: {fn_df['prob'].mean():.3f}")

        # Silent vs vocal FNs (only if kchi features are available)
        if "kchi_total_dur" in fn_df.columns:
            ...
            print("\n(kchi_total_dur not available — silent/vocal FN breakdown skipped)")
    ...
    for child_id, sub in df.groupby("child_id"):
        ...
        # Add diarizer features if available
        if "kchi_total_dur" in sub.columns:
            row["mean_kchi_dur"] = sub["kchi_total_dur"].mean()
        if "phon_n_unique" in sub.columns:
            row["mean_phon_unique"] = sub["phon_n_unique"].mean()
    ...
    display_cols = ["child_id", "timepoint", "n_clips", "accuracy", "n_fp", "n_fn", "mean_prob"]
    for optional_col in ["mean_kchi_dur", "mean_phon_unique", "mean_n_children", "pct_interaction"]:
        if optional_col in child_df.columns:
    ...
    for _, row in errors.head(15).iterrows():
        ...
        # Print diarizer features if available
        feat_parts = [f"task={row['task']}"]
        for col in ["kchi_total_dur", "kchi_n_segments", "phon_n_total", "phon_n_unique"]:
            if col in row.index and pd.notna(row[col]):
                feat_parts.append(f"{col}={row[col]:.2f}" if isinstance(row[col], float) else f"{col}={row[col]}")
        print(f"    {' | '.join(feat_parts)}")
    ...
    # Silent FN count (only if kchi features available)
    if "kchi_total_dur" in fn_df.columns and len(fn_df) > 0:
        silent_fn_count = int((fn_df["kchi_total_dur"] == 0).sum())
    else:
        silent_fn_count = -1  # sentinel: not available
    ...
    summary = {
        ...
        "fn_silent_child_pct": silent_fn_count / max(n_fn, 1) if silent_fn_count >= 0 else "N/A",
        "fn_vocal_child": int(n_fn - silent_fn_count) if silent_fn_count >= 0 else "N/A",
        ...
        "mean_fp_prob": float(fp_df["prob"].mean()) if len(fp_df) > 0 else 0,
        "mean_fn_prob": float(fn_df["prob"].mean()) if len(fn_df) > 0 else 0,
    }

    # Add diarizer-specific summary fields if available
    if "kchi_total_dur" in df.columns:
        summary["mean_fp_kchi_dur"] = float(fp_df["kchi_total_dur"].mean()) if len(fp_df) > 0 else 0
        summary["mean_fn_kchi_dur"] = float(fn_df["kchi_total_dur"].mean()) if len(fn_df) > 0 else 0
